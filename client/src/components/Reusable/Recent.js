...
export function Recent({ title, showAll, category, SearchResults, padding, headerPadding, titleStyle }) {
  ...
  //Fn to rename file
  const handleRename = async (e, name, item) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  return (
    ...
    >
      ...
      {share && <SharePopUp isOpen={share} link={link} width={"w-11/12 md:w-4/12"} />}
      <h1
        className={`text-blue-700 text-xl font-extrabold ${titleStyle && titleStyle} ${
                    ...
                    {dropdownState.includes(item) && (
                      <div className='origin-top-right absolute right-0 mt-2 w-36 md:w-40 rounded-md shadow-lg bg-slate-300 ring-1 ring-black ring-opacity-5 z-50'>
                        ...
                        >
                          ...
                          >
                            <span>Delete</span>
                            <i className={`fas fa-trash text-xs text-red-700 absolute right-3`}></i>
                          </button>
                          <button
                          ...
                          >
                            <span className='text-left'>Auto Download Link</span>
